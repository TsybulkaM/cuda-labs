...
SyntheticPattern
InputArgsParser::stringToSyntheticPattern(const std::string &str) {
  if (str == "checkerboard")
...
FilterOptions InputArgsParser::parseArgs() {
  cxxopts::Options options("cuda-webcam-filter",
                           "Real-time webcam filter with CUDA acceleration");
  ...
  if (result.count("version")) {
    std::cout << "CUDA Webcam Filter version " << CUDA_WEBCAM_FILTER_VERSION
              << std::endl;
    exit(0);
  }
...
void InputArgsParser::setupOptions(cxxopts::Options &options) {
  options.add_options()(
      ...
      "s,synthetic",
      "Synthetic pattern type: 'checkerboard', 'gradient', 'noise'",
  ...
  options.add_options()("save",
                        ...
                        "video when input is video/webcam)",
