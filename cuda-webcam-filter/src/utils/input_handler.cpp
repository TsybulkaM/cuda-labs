...
InputHandler::InputHandler(const FilterOptions &options)
  ...
  case InputSource::WEBCAM:
    ...
    if (!m_cap.isOpened()) {
      PLOG_ERROR << "Failed to open webcam with device ID: "
                 << m_options.deviceId;
    }
    break;
...
double InputHandler::getFPS() const {
  if (m_options.inputSource == InputSource::VIDEO ||
      m_options.inputSource == InputSource::WEBCAM) {
...
cv::Size InputHandler::getFrameSize() const {
  if (m_options.inputSource == InputSource::IMAGE) {
    if (!m_staticFrame.empty())
      return m_staticFrame.size();
    return cv::Size(0, 0);
  }
...
bool InputHandler::isStaticImage() const { return m_isStaticImage; }
...
void InputHandler::displaySideBySide(const cv::Mat &frame1,
                                     const cv::Mat &frame2) {
  cv::Mat combined;
