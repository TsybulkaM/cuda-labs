    ...
    for (int j = -halfFilter; j <= halfFilter; j++) {
        for (int i = -halfFilter; i <= halfFilter; i++) {
            ...
            for (int c = 0; c < channels; c++)
                sum[c] += input[imgIdx + c] * filter[filterIdx];
        }
    }
...
// Block 16x16, max filter 9x9 (halo=4) → tile 24x24, max 4 channels = 2304 bytes shared
__global__ void convolutionKernelShared(const unsigned char *input,
                                        ...
                                        const float *filter,
                                        const int filterWidth) {
    __shared__ unsigned char tile[24][24][4];
    ...
    // Cooperatively load the tile (block region + halo) into shared memory.
    // Each thread may load more than one pixel when the tile is larger than the block.
    ...
    for (int j = -halfFilter; j <= halfFilter; j++) {
        for (int i = -halfFilter; i <= halfFilter; i++) {
            int filterIdx = (j + halfFilter) * filterWidth + (i + halfFilter);
            for (int c = 0; c < channels; c++)
                sum[c] += tile[ty + halfFilter + j][tx + halfFilter + i][c] * filter[filterIdx];
        }
    }
...
void convolutionGPU(const Image *input, Image *output, const std::span<const float> filter) {
    ...
    CUDA_CHECK(cudaMemcpyToSymbol(d_filter, filter.data(), filter.size() * sizeof(float)));
    ...
    convolutionKernelNaive<<<grid, block>>>(
        d_input, d_output,
...
void convolutionGPUShared(const Image *input, Image *output, const std::span<const float> filter) {
    size_t imageSize = input->width * input->height * input->channels;
    int filterWidth = (int)std::sqrt((double)filter.size());
    ...
    dim3 block(16, 16);
    dim3 grid((input->width + 15) / 16, (input->height + 15) / 16);
    convolutionKernelShared<<<grid, block>>>(
        d_input, d_output,
        input->width, input->height, input->channels,
        d_filter, filterWidth);
    CUDA_CHECK(cudaGetLastError());
